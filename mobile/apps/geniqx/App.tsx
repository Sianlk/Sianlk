...
function HomeScreen({ navigation }: any) {
  ...
  const ITEMS: [string, string, string][] = [["\u269b\ufe0f", "Circuit Builder", "Visual quantum circuit editor"], ["\ud83e\uddee", "Quantum Sim", "Run your circuits"], ["\ud83e\udd16", "Quantum AI", "AI with quantum advantage"], ["\ud83d\udcca", "Visualiser", "Results & probability charts"]];
  return (<View style={ flex: 1, backgroundColor: T.bg }><LinearGradient colors={['#7C3AED22', '#0A0A0F', '#08080E']} style={StyleSheet.absoluteFill} /><Particles count={16} accent="{color}" /><SafeAreaView style={ flex: 1 }><Animated.ScrollView style={ opacity: fade } contentContainerStyle={ padding: 22, paddingBottom: 40 }><Text style={ color: T.text, fontSize: 26, fontWeight: '800', marginBottom: 4 }>{'GeniQX 🔬'}</Text><Text style={ color: T.muted, marginBottom: 22, fontSize: 14 }>{'Run quantum circuits, quantum AI experiments, visualise results'}</Text><LinearGradient colors={['#7C3AED', T.cyan]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={ borderRadius: 20, padding: 22, marginBottom: 20 }><Text style={ color: '#fff', fontSize: 18, fontWeight: '800', marginBottom: 4 }>{'Quantum Research'}</Text><Text style={ color: 'rgba(255,255,255,0.82)', fontSize: 14 }>{'Run quantum circuits, quantum AI experiments, visualise results'}</Text></LinearGradient>{ITEMS.map(([icon, label, sub]: any, i: number) => (<TouchableOpacity key={i} style={ marginBottom: 10 }><GCard style={ padding: 16 }><View style={ flexDirection: 'row', alignItems: 'center', gap: 12 }><Text style={ fontSize: 26 }>{icon}</Text><View style={ flex: 1 }><Text style={ color: T.text, fontWeight: '600', fontSize: 14 }>{label}</Text><Text style={ color: T.muted, fontSize: 12 }>{sub}</Text></View><Text style={ color: T.muted }>›</Text></View></GCard></TouchableOpacity>))}</Animated.ScrollView></SafeAreaView></View>);
}
