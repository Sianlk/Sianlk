...
function SplashScreen({ navigation }: any) {
  ...
  return (<View style={ flex: 1, backgroundColor: T.bg, alignItems: 'center', justifyContent: 'center' }><LinearGradient colors={['#10B98144', '#0A0A0F', '#0A0A0F']} style={StyleSheet.absoluteFill} /><Particles count={20} accent='#10B981' /><Animated.View style={ alignItems: 'center', transform: [{ scale }], opacity: op }><LinearGradient colors={['#10B981', T.cyan]} style={ width: 110, height: 110, borderRadius: 30, alignItems: 'center', justifyContent: 'center' }><Text style={ fontSize: 52 }>{'🖥️'}</Text></LinearGradient><Text style={ color: T.text, fontSize: 32, fontWeight: '900', marginTop: 20 }>{'TerminalAI'}</Text><Text style={ color: T.muted, fontSize: 14, marginTop: 6 }>{'AI Terminal'}</Text></Animated.View></View>);
}

function OnboardingScreen({ navigation }: any) {
  const [idx, setIdx] = useState(0); const anim = useRef(new Animated.Value(1)).current;
  const SLIDES_G = [ { emoji: '🖥️', title: 'AI Terminal', desc: 'Command explanations, smart suggestions, shell automation' }, { emoji: '⚡', title: 'Quantum-Powered', desc: 'Advanced AI algorithms give you capabilities no other app can match.' }, { emoji: '🚀', title: 'Built for Scale', desc: 'Used by thousands of professionals. No limits, no compromises.' } ];
  ...
  return (<View style={ flex: 1, backgroundColor: T.bg }><LinearGradient colors={['#10B98122', '#0A0A0F']} style={StyleSheet.absoluteFill} /><SafeAreaView style={ flex: 1 }><View style={ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 32 }><Animated.View style={ alignItems: 'center', opacity: anim }><Text style={ fontSize: 88 }>{s.emoji}</Text><Text style={ color: T.text, fontSize: 28, fontWeight: '800', textAlign: 'center', marginTop: 16, marginBottom: 14 }>{s.title}</Text><Text style={ color: T.muted, fontSize: 16, textAlign: 'center', lineHeight: 26 }>{s.desc}</Text></Animated.View></View><View style={ padding: 28 }><View style={ flexDirection: 'row', justifyContent: 'center', gap: 8, marginBottom: 26 }>{SLIDES_G.map((_, i) => <View key={i} style={ width: i === idx ? 22 : 6, height: 6, borderRadius: 3, backgroundColor: i === idx ? '#10B981' : T.border } />)}</View><GBtn label={idx < SLIDES_G.length - 1 ? 'Next →' : 'Get Started Free'} onPress={advance} accent="{color}" /></View></SafeAreaView></View>);
}

function HomeScreen({ navigation }: any) {
  const fade = useRef(new Animated.Value(0)).current;
  useEffect(() => { Animated.timing(fade, { toValue: 1, duration: 600, useNativeDriver: true }).start(); }, []);
  const ITEMS: [string, string, string][] = [["\ud83d\udcac", "Explain Command", "AI command explanation"], ["\ud83d\udd2e", "Suggest", "Smart suggestions"], ["\ud83d\udcdc", "History", "Command history + AI"], ["\ud83e\udd16", "Shell AI", "Natural language shell"]];
  return (<View style={ flex: 1, backgroundColor: T.bg }><LinearGradient colors={['#10B98122', '#0A0A0F', '#08080E']} style={StyleSheet.absoluteFill} /><Particles count={16} accent="{color}" /><SafeAreaView style={ flex: 1 }><Animated.ScrollView style={ opacity: fade } contentContainerStyle={ padding: 22, paddingBottom: 40 }><Text style={ color: T.text, fontSize: 26, fontWeight: '800', marginBottom: 4 }>{'TerminalAI 🖥️'}</Text><Text style={ color: T.muted, marginBottom: 22, fontSize: 14 }>{'Command explanations, smart suggestions, shell automation'}</Text><LinearGradient colors={['#10B981', T.cyan]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={ borderRadius: 20, padding: 22, marginBottom: 20 }><Text style={ color: '#fff', fontSize: 18, fontWeight: '800', marginBottom: 4 }>{'Developer Tools'}</Text><Text style={ color: 'rgba(255,255,255,0.82)', fontSize: 14 }>{'Command explanations, smart suggestions, shell automation'}</Text></LinearGradient>{ITEMS.map(([icon, label, sub]: any, i: number) => (<TouchableOpacity key={i} style={ marginBottom: 10 }><GCard style={ padding: 16 }><View style={ flexDirection: 'row', alignItems: 'center', gap: 12 }><Text style={ fontSize: 26 }>{icon}</Text><View style={ flex: 1 }><Text style={ color: T.text, fontWeight: '600', fontSize: 14 }>{label}</Text><Text style={ color: T.muted, fontSize: 12 }>{sub}</Text></View><Text style={ color: T.muted }>›</Text></View></GCard></TouchableOpacity>))}</Animated.ScrollView></SafeAreaView></View>);
}

function MainTabs() {
  return (<Tab.Navigator screenOptions={ headerShown: false, tabBarStyle: { backgroundColor: '#0F0F17', borderTopColor: T.border, height: Platform.OS === 'ios' ? 88 : 64, paddingBottom: Platform.OS === 'ios' ? 28 : 10, paddingTop: 8 }, tabBarActiveTintColor: '#10B981', tabBarInactiveTintColor: T.muted, tabBarLabelStyle: { fontSize: 11, fontWeight: '600' } }><Tab.Screen name="Home" component={HomeScreen} options={ tabBarIcon: ({ color }) => <Text style={ fontSize: 22, color }>🏠</Text>, tabBarLabel: 'Home' } /></Tab.Navigator>);
}
